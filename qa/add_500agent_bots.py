...
AGENTS = [
    ...
    dict(id="career-parser", type="agent", name="Career Parser", fleet="career",
         ...
         model="deepseek-v4-flash", capabilities=["career-ops", "resume-parse"],
         budget={"max_runs_per_day": 5, "max_cost_est": 3000},
         notes="Parse cv.md -> structured profile JSON + ATS fit. Pass --job-desc to score vs a JD."),
    dict(id="career-applier", type="agent", name="Career Applier", fleet="career",
         ...
         notes="Tailored cover letter + resume bullets + interview Qs. Pass --job-desc and --candidate."),
    ...
    dict(id="career-brand", type="agent", name="Career Brand", fleet="career",
         ...
         budget={"max_runs_per_day": 3, "max_cost_est": 3000},
         notes="LinkedIn personal-brand posts for job search. Pass post topic/brand voice."),
    ...
    dict(id="career-pii", type="agent", name="Career PII Guard", fleet="career",
         ...
         agent_args="--local", model=None, capabilities=["career-ops", "pii-redaction"],
    ...
    dict(id="news-digest", type="agent", name="News Digest", fleet="research",
         ...
         model="deepseek-v4-flash", capabilities=["news-summary"],
         budget={"max_runs_per_day": 1, "max_cost_est": 3000},
         notes="Daily news digest. Change --topic via bot config."),
    dict(id="market-watch", type="agent", name="Market Watch", fleet="research",
         ...
         agent_args="--ticker SPY", model="deepseek-v4-flash",
         capabilities=["market-research"],
         budget={"max_runs_per_day": 1, "max_cost_est": 3000},
         notes="Weekly macro gauge for weather-arb context. Pass --ticker."),
    dict(id="arb-debate", type="agent", name="Arb Debate", fleet="research",
         schedule="manual", agent_dir="20-multi-agent-debate",
         agent_args="--rounds 2", model="deepseek-v4-flash",
         capabilities=["hypothesis-testing"],
         budget={"max_runs_per_day": 3, "max_cost_est": 6000},
         notes="Multi-agent debate for weather-arb hypothesis testing. Pass --topic."),
    ...
    dict(id="data-analyst", type="agent", name="Data Analyst", fleet="qa",
         ...
         budget={"max_runs_per_day": 5, "max_cost_est": 6000},
         notes="CSV/Excel analysis for backtests + admin analytics. Pass --file. (Sandbox the exec.)"),
    ...
    dict(id="meeting-scribe", type="agent", name="Meeting Scribe", fleet="ops",
         ...
         budget={"max_runs_per_day": 5, "max_cost_est": 4000},
         notes="Turn a transcript into notes + action items. Pass transcript text."),
